...
export default function WithdrawalPage() {
  ...
  const availableBalance = useMemo(() => {
    if (userVaultBalance && typeof userVaultBalance === 'bigint') {
      return formatUnits(userVaultBalance, selectedTokenDecimals)
    }
    return '0'
  }, [userVaultBalance, selectedTokenDecimals])
  const availableBalanceDisplay = useMemo(() => {
    try {
      const num = parseFloat(availableBalance || '0')
      return (num >= 1000 ? num.toFixed(2) : num.toFixed(6)).replace(/\.?0+$/, '') || '0.00'
    } catch {
      return '0.00'
    }
  }, [availableBalance])
  const disabledReason = useMemo(() => {
    ...
    if (!hasSufficientBalance) return `Insufficient vault balance. Available: ${availableBalanceDisplay} ${selectedSymbol}`
    if (showRecipientInput && recipientAddress && recipientAddress.trim() !== '' && !/^0x[a-fA-F0-9]{40}$/.test(recipientAddress)) {
      return 'Invalid recipient address.'
    }
    return ''
  }, [isConnected, amount, withdrawalAmount, hasSufficientBalance, availableBalanceDisplay, selectedSymbol, showRecipientInput, recipientAddress])
  ...
  // availableBalance computed above as numeric string → availableBalanceDisplay for UI
              ...
              <div className="mb-8">
                ...
                {isLoadingVaultBalance ? (
                  ...
                  <p className="text-4xl font-bold text-white break-words overflow-wrap-anywhere">
                    {availableBalanceDisplay} {selectedSymbol}
                  </p>
                )}
              </div>
              ...
              {currency === 'ngn' ? (
                ...
                <>
                  ...
                  <div className="mb-4">
                    ...
                    {/* Hide on-page contract addresses for cleaner UX */}
                  </div>
                  ...
                  <div className="mb-6">
                    ...
                    <p className={`text-xs mt-1 ${availableBalanceDisplay === '0' || availableBalanceDisplay === '0.00' ? 'text-red-400' : 'text-white/50'}`}>
                      Available: {availableBalanceDisplay} {selectedSymbol}
                    </p>
                    {exceedsBalance && (
                      <p className="text-xs text-red-400 mt-1">
                        Amount exceeds available vault balance. Available: {availableBalanceDisplay} {selectedSymbol}
                      </p>
                    )}
                  </div>
